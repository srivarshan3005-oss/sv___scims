...
public ComplaintResponse createComplaint(
    ...
    log.info("=== CREATE COMPLAINT STARTED ===");
    ...
    try {
        ...
        log.info("Complaint saved successfully. ID={}", saved.getId());
        ...
        if (image != null && !image.isEmpty()) {
            ...
            String imagePath =
                    fileUploadService.uploadImage(image, "complaints");
        ...
        log.info("Status history saved.");
        log.info("=== CREATE COMPLAINT COMPLETED ===");

        return mapToResponse(saved);

    } catch (Exception ex) {

        log.error("ERROR WHILE CREATING COMPLAINT", ex);
    ...
    private ComplaintResponse mapToResponse(Complaint c) {
        ...
        List<String> imageUrls = images.stream()
                // imagePath is already "complaints/uuid.jpg"
                // prefix with /uploads/ to match the static resource handler
