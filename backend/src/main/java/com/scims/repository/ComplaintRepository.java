...
public interface ComplaintRepository extends JpaRepository<Complaint, Long> {
    ...
    /**
     * Same filter logic as findAllWithFilters but scoped to a single
     * department — used by Sub Admins, who must never see complaints
     * outside their own department.
     */
    @Query("SELECT c FROM Complaint c WHERE " +
           "c.department.id = :departmentId AND " +
           "(:status IS NULL OR c.status = :status) AND " +
           "(:categoryId IS NULL OR c.category.id = :categoryId) AND " +
           "(:search IS NULL OR (LOWER(c.title) LIKE LOWER(CONCAT('%',:search,'%')) " +
           " OR LOWER(c.location) LIKE LOWER(CONCAT('%',:search,'%'))))")
    Page<Complaint> findAllByDepartmentWithFilters(
