...
export default function SubAdminDashboard() {
  ...
  useEffect(() => {
    Promise.all([
      subAdminComplaintAPI.getDashboard(),
      subAdminComplaintAPI.getAll({ page: 0, size: 8, status: 'PENDING' }),
    ])
    ...
    subAdminComplaintAPI.getAll({ page: 0, size: 200 })
      .then(r => {
  ...
  if (loading) return (
    <Layout role="subadmin" title="Sub Admin Dashboard">
  ...
  return (
    <Layout role="subadmin" title="Sub Admin Dashboard"
      subtitle={user?.departmentName ? `${user.departmentName} — department overview` : 'Department overview'}>
        ...
        <div className="card mb-4">
          ...
          <div className="card-body p-0" style={{ borderRadius: '0 0 0.5rem 0.5rem', overflow: 'hidden' }}>
            <Suspense fallback={<div className="p-3 text-muted small">Loading map…</div>}>
              <ComplaintMap complaints={mapComplaints} height="320px" zoom={12} />
            </Suspense>
          </div>
        </div>
      )}
        ...
        <div className="col-lg-8">
          <div className="card">
            <div className="card-header d-flex justify-content-between align-items-center">
              ...
              <Link to="/subadmin/complaints?status=PENDING" className="btn btn-sm btn-outline-warning">
                View All Pending
              </Link>
            </div>
                    ...
                    <tbody>
                      {complaints.map(c => (
                        <tr key={c.id}>
                          ...
                          <td>
                            <Link to={`/subadmin/complaints/${c.id}`} className="btn btn-sm btn-primary">
          ...
          <div className="card mb-4">
            ...
            <div className="card-body">
              <div className="d-grid gap-2">
                <Link to="/subadmin/complaints" className="btn btn-outline-primary text-start">
                  <i className="bi bi-clipboard-data me-2"></i>Manage Department Complaints
                </Link>
              </div>
            </div>
          </div>
