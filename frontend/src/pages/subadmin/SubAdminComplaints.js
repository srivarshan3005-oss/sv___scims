...
import { subAdminComplaintAPI, categoryAPI } from '../../api';
...
export default function SubAdminComplaints() {
    ...
    useEffect(() => {
        // Categories list is global (not department-filtered) — the
        // category dropdown only matters within complaints already scoped
        // to this Sub Admin's department by the backend, so any category
        // not in their department simply returns zero rows if selected.
        categoryAPI.getActive()
            .then(r => setCategories((r.data.data || []).filter(c =>
    ...
    const fetchComplaints = useCallback(async (p = 0) => {
        ...
        try {
            ...
            const res = await subAdminComplaintAPI.getAll(params);
    ...
    return (
        <Layout role="subadmin" title="Department Complaints"
            subtitle={`${totalElements} complaints${user?.departmentName ? ` in ${user.departmentName}` : ''}`}>
            {error && (
