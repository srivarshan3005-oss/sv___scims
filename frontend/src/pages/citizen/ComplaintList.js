...
export default function ComplaintList() {
  ...
  const fetchComplaints = useCallback(async (p = 0) => {
    ...
    try {
      ...
      // Apply status filter client-side on the returned page.
      // Citizens typically have a small number of complaints so this is acceptable.
      // The citizen complaints endpoint does not expose server-side status filtering.
      const filtered = statusFilter
        ? content.filter(c => c.status === statusFilter)
        : content;
      setComplaints(filtered);
      setTotalPages(data.totalPages);
      // When filtering client-side, show the actual unfiltered total so pagination is consistent.
        ...
        <div className="card-body p-0">
          {loading ? (
            ...
            <div className="text-center py-5">
              ...
              {statusFilter && (
                <button className="btn btn-sm btn-outline-secondary me-2" onClick={() => setStatusFilter('')}>
                  Clear Filter
                </button>
              )}
              <Link to="/citizen/complaints/new" className="btn btn-primary btn-sm">
