...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate(role === 'ROLE_ADMIN' ? '/admin/dashboard' : '/citizen/dashboard', { replace: true });
    } catch (err) {
    ...
    <div className="auth-wrapper">
      <div className="auth-card">
        ...
        <div className="d-flex gap-2 mb-3">
          <button type="button" className="btn btn-sm btn-outline-secondary flex-1 w-50" onClick={() => fillDemo('admin')}>
            <i className="bi bi-shield-fill me-1"></i>Demo Admin
          </button>
          <button type="button" className="btn btn-sm btn-outline-secondary flex-1 w-50" onClick={() => fillDemo('citizen')}>
            <i className="bi bi-person me-1"></i>Demo Citizen
          </button>
        </div>
